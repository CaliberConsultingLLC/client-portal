    ...
    <div className="grid gap-6 lg:grid-cols-3">
      ...
      <Card title="Incoming CDRS" subtitle="How others rate each department">
        ...
        <p className="mt-2 text-center text-xs text-text-muted">
          DWS Average: {data.meta.dwsAverageIncoming.toFixed(2)}
        </p>
      </Card>
      ...
      <Card title="Collaboration Index" subtitle="Deeper department-specific measure">
        ...
        />
        <p className="mt-2 text-center text-xs text-text-muted">
          DWS Average: {data.meta.dwsAverageOverall.toFixed(1)}
        </p>
      </Card>
    </div>
  );
...
function HeatmapTab() {
  ...
  for (const dept of data.meta.departments) {
    ...
    // Column total = outgoing CDRS for that column (what that column received)
