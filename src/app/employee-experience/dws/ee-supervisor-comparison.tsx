...
type Data = {
  ...
  index: { id: string; name: string; statements: StatementRow[] };
};
...
export function EESupervisorComparison({
      ...
      supervisors
        .map((supervisor) => {
          const values = statements
            .map((statement) => statement.bySup[supervisor.id]?.current)
            .filter((value): value is number => typeof value === "number" && value > 0);
          return {
            id: supervisor.id,
            name: supervisor.name,
            responses: supervisor.responses,
            score: values.length > 0 ? round1(average(values)) : 0,
          };
        })
  ...
  const orgAverage = useMemo(() => {
    if (supervisorScores.length === 0) return 0;
    return round1(average(supervisorScores.map((row) => row.score)));
  }, [supervisorScores]);
  ...
  return (
    <div className="block" style={EE_PERSPECTIVE_CANVAS_STYLE}>
        ...
        <div style={{ maxWidth: 1320, margin: "0 auto", width: "100%" }} className="flex flex-col gap-5">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-0.5 font-semibold" style={{ fontSize: 14, color: "#3B4B63" }}>
                  {data.current.labelLong} · Leadership index only
                </p>
              </div>
          ...
          >
            <div className="px-6 py-4" style={{ borderBottom: "1px solid #E2E8EF" }}>
              <h3 className="font-bold" style={{ fontSize: 15, color: "#152238" }}>Leadership Statement Heat Map</h3>
              <p className="mt-1 text-[12px]" style={{ color: "#6E7E96" }}>
                Rows are Leadership statements and columns are supervisors for the selected campaign.
              </p>
            </div>
            <div className="px-6 py-5">
                ...
                >
                  <colgroup>
                    ...
                    {supervisorScores.map((row) => (
                      <col key={`col-${row.id}`} style={{ width: 86 }} />
                    ))}
                    <col style={{ width: 78 }} />
                  </colgroup>
                  <thead>
                  ...
                  <tbody>
                    {statements.map((statement) => {
                      const values = supervisorScores
                        .map((row) => statement.bySup[row.id]?.current)
                        .filter((value): value is number => typeof value === "number" && value > 0);
                      const statementAvg = values.length > 0 ? round1(average(values)) : 0;
      ...
      <aside className="hidden xl:flex xl:flex-col xl:gap-4 xl:p-6" style={EE_GUIDANCE_RAIL_STYLE}>
        <EEContextRail scale={data.scale} howToRead="Leadership-only view: top bars show each supervisor's current average and the heat map shows statement-level current scores for each supervisor." />
        {dashboardInstanceId ? (
          <GuidancePinRail
